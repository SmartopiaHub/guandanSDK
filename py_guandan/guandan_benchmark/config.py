...
def validate_api_key_format(api_key: str) -> Optional[str]:
    ...
    # Current format: sk-zq-{keyId}_{secret} (keyId: 8 alphanumeric chars,
    # as generated by the lobby's developer API key service).
    if api_key.startswith("sk-zq-"):
        payload = api_key[6:]  # strip 'sk-zq-'
        if '_' not in payload:
            return (
                "API key is missing '_' separator between public key ID "
                "and secret. Expected format: sk-zq-{keyId}_{secret}"
            )
        key_id, secret = payload.split('_', 1)
        if len(key_id) != 8 or not key_id.isalnum():
            return (
                f"API key public key ID '{key_id}' looks wrong. "
                "Expected format: sk-zq-{keyId}_{secret} with an "
                "8-alphanumeric key ID"
            )
        if len(secret) < 32:
            return (
...
def load_config(
    ...
    # -- name ------------------------------------------------------------------
    # Optional display name for the benchmark run (shown in the Developer
    # Center history).  Absent/empty means the server picks a default.
